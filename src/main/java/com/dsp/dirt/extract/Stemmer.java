 ...
 * Practical normalization: stem letters-only to keep behavior stable on syntactic-ngram tokens.
 */
public class Stemmer {

    /** Stem a single token safely. */
    public String stem(String word) {
        if (word == null) return "";
        word = word.trim().toLowerCase(Locale.ROOT);
        if (word.isEmpty()) return "";

        // Keep letters only (Porter is defined on alphabetic tokens).
        // If nothing left, fall back to the lowercased original.
        String cleaned = keepLettersOnly(word);
        if (cleaned.isEmpty()) return word;

        Porter porter = new Porter();
        porter.add(cleaned.toCharArray(), cleaned.length());
        porter.stem();
    ...
    /**
     * Minimal Porter implementation (no main(), no file I/O).
     * Based on the standard Porter stemmer algorithm by Martin Porter.
     */
    private static final class Porter {
        private char[] b = new char[50];
        private int i = 0;      // offset into b
        private int i_end = 0;  // end of stemmed word
        private int j, k;
